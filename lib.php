...
function sandbox_refresh_events($courseid = 0) {
    ...
    if ($courseid == 0) {
        if (!$sandboxes = $DB->get_records('sandbox')) {
            return true;
        }
    } else {
        if (!$sandboxes = $DB->get_records('sandbox', ['course' => $courseid])) {
            return true;
        }
    }
...
/**
 * This function updates the events associated to the sandbox.
 * If $override is non-zero, then it updates only the events
 * associated with the specified override.
 *
 * @param sandbox $sandbox the sandbox object.
 * @param object $override (optional) limit to a specific override.
 */
function sandbox_update_events($sandbox, $override = null) {
}
...
function sandbox_delete_instance($id) {
    ...
    if (!$sandbox = $DB->get_record('sandbox', ['id' => $id])) {
        return false;
    }

    // Delete any dependent records here.
    $DB->delete_records('sandbox', ['id' => $sandbox->id]);
...
function sandbox_scale_used($sandboxid, $scaleid) {
    global $DB;
    if ($scaleid && $DB->record_exists('sandbox', ['id' => $sandboxid, 'grade' => -$scaleid])) {
        return true;
    } else {
        return false;
    }
...
function sandbox_scale_used_anywhere($scaleid) {
    global $DB;
    if ($scaleid && $DB->record_exists('sandbox', ['grade' => -$scaleid])) {
        return true;
    } else {
        return false;
    }
 ...
 */
function sandbox_grade_item_update(stdClass $sandbox) {
    global $CFG;
 ...
 */
function sandbox_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=[]) {
    global $DB, $CFG;
